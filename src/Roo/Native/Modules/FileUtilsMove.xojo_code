#tag Class
Protected Class FileUtilsMove
Implements Roo.Invokable,Roo.Textable
	#tag Method, Flags = &h0
		Function Arity() As Variant
		  ' Part of the Invokable interface.
		  ' Return the number of parameters the `move` function requires.
	...
	#tag Method, Flags = &h21
		Private Function DoMove(source As FolderItem, destination As FolderItem, overwrite As Boolean) As Roo.Objects.BooleanObject
		  ' Assumes `source` and `destination` both exist and that `destination` is a folder.
		  
		  Using FileSystem
		  
		  ' Use our FileSystem module to robustly move `source` to `destination`.
		  Dim e As Error = source.MoveTo(destination, overwrite)
	...
	#tag Method, Flags = &h0
		Function Invoke(interpreter as Interpreter, arguments() as Variant, where as Roo.Token) As Variant
		  ' FileUtils.move(source, destination, overwrite as Boolean) as Boolean object
		  ...
		  If Not arguments(0) IsA TextObject And Not arguments(0) IsA FileObject Then
		    Raise New RuntimeError(where, "The FileUtils.move(source, destination, overwrite) method " + _
		  ...
		  If arguments(0) IsA FileObject Then
		    If FileObject(arguments(0)).File = Nil Then
		      ...
		      "FileUtils.move(source, destination, overwrite) method.")
		    End If
		    source = FileObject(arguments(0)).File
		  Else
		    source = Roo.RooPathToFolderItem(TextObject(arguments(0)).Value, where.File)
		    If source = Nil Then
		      Raise New RuntimeError(where, "Invalid source path (" + TextObject(arguments(0)).Value + _
		      ") passed to the FileUtils.move(source, destination, overwrite) method.")
		    End If
		  End If
		  ...
		  Else
		    ...
		    If destination = Nil Then
		      Raise New RuntimeError(where, "Invalid destination path (" + TextObject(arguments(1)).Value + _
		      ") passed to the FileUtils.move(source, destination, overwrite) method.")
		    End If
		  End If
		  ...
		  If Not destination.Exists Then
		    ...
		    "FileUtils.move(source, destination, overwrite) method. The destination does not exist.")
		  End If
		  
		  ' The destination must always be a folder. Check this.
		  If Not destination.Directory Then
		    Raise New RuntimeError(where, "Invalid destination passed to the " + _
		    "FileUtils.move(source, destination, overwrite) method. The destination must be a folder.")
		  End If
		  
		  ' We now have FolderItems representing the source and destination. 
		  ' Use our helper method to actually do the move.
		  Return DoMove(source, destination, overwrite)
	...
	#tag Method, Flags = &h0
		Function ToText(interpreter As Roo.Interpreter = Nil) As String
		  ...
		  Return "<function: FileUtils.move>"
		End Function
	#tag EndMethod
