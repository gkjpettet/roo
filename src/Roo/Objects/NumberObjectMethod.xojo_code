#tag Class
Protected Class NumberObjectMethod
Implements Roo.Invokable,Roo.Textable
	...
	#tag Method, Flags = &h0
		Sub Constructor(parent as NumberObject, name as String)
		  self.parent = parent
		  self.name = name
		End Sub
	#tag EndMethod
	...
	#tag Method, Flags = &h21
		Private Function DoRespondsTo(arguments() as Variant, where as Token) As BooleanObject
		  ' Number.responds_to?(what as Text) as Boolean.
		  ...
		  if Lookup.NumberGetter(what) then
		    return new BooleanObject(True)
		  elseif Lookup.NumberMethod(what) then
		    return new BooleanObject(True)
		  else
