#tag Class
Protected Class BooleanObjectMethod
Implements Roo.Invokable,Roo.Textable
	...
	#tag Method, Flags = &h0
		Sub Constructor(parent as BooleanObject, name as String)
		  self.parent = parent
		  self.name = name
		End Sub
	#tag EndMethod
	...
	#tag Method, Flags = &h21
		Private Function DoRespondsTo(arguments() as Variant, where as Token) As BooleanObject
		  ' Boolean.responds_to?(what as Text) as Boolean.
		  ...
		  if Lookup.BooleanGetter(what) then
		    return new BooleanObject(True)
		  elseif Lookup.BooleanMethod(what) then
		    return new BooleanObject(True)
		  else
