#tag Class
Protected Class RooSLFileSystemCopy
Implements Invokable, Stringable
	...
	#tag Method, Flags = &h21
		Private Function Copy(source As FolderItem, destination As FolderItem, overwrite As Boolean, where As RooToken) As RooBoolean
		  // Internal helper method.
		  // Assumes `source` and `destination` both exist and that `destination` is a folder.
		  
		  // Use this interpreter's FileSystem module to copy `source` to `destination`.
		  Dim e As RooSLFileSystem.Error = Owner.Copy(source, destination, overwrite)
		  ...
		  Case RooSLFileSystem.Error.AttemptToDeleteProtectedFolderItem
		    // This copy would have resulted in the deletion of a protected 
	...
	#tag Method, Flags = &h0
		Function Invoke(interpreter As RooInterpreter, args() As Variant, where As RooToken) As Variant
		  ...
		  // FileSystem.copy(source, destination, overwrite as Boolean) as Boolean.
		  ...
		  // Ensure that `source` is a Stringable or File object.
		  ...
		  // Ensure that `destination` is a Stringable or File object.
		  ...
		  If Not destination.Directory Then
		    Raise New RooRuntimeError(where, "Invalid destination parameter (" + _
		    destination.NativePath + "). The destination must be a folder.")
		  End If
		  
		  // We now have FolderItems representing the source and destination. 
		  Return Copy(source, destination, overwrite, where)
	...
	#tag Method, Flags = &h0
		Function StringValue() As String
		  ...
		  Return "<function: FileSystem.copy>"
		End Function
	#tag EndMethod
