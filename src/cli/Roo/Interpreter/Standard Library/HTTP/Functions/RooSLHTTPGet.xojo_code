#tag Class
Protected Class RooSLHTTPGet
Implements Invokable, Stringable
	#tag Method, Flags = &h0
		Function Arity() As Variant
		  // Part of the Invokable interface.
		  
		  // The HTTP.get() method takes 1 or 2 parameters.
		  Return Array(1, 2)
		End Function
	#tag EndMethod
	...
	#tag Method, Flags = &h0
		Function Invoke(interpreter As RooInterpreter, args() As Variant, where As RooToken) As Variant
		  ...
		  // HTTP.get(url as Text) as HTTP.Request
		  // HTTP.get(url as Text, timeout as Integer) as HTTP.Request
		  ...
		  r.URL = url
		  r.Timeout = timeout
		  r.Method = RooSLHTTP.MethodType.GET
